...
version
library()                                          #显示库中所有可用的包
pkgList <- as.data.frame(installed.packages("/Library/Frameworks/R.framework/Versions/3.5/Resources/library"), stringsAsFactors = F)
...
search()                                           #显示哪些库已加载并可使用
if(!require(bench)) í
packageDescription("tidyverse")                    #查看library的详细信息
help(package ="tidyverse"); ??tidyverse;
update.packages("tidyverse")                       #更新包
...
# how to change your default python version for R
...
#在一个R会话(session)结束时，你可以将当前工作空间保存到一个镜像中(.RData)，并在下次启动R时自动载入它
ls()                  #列出当前工作空间中的对象
...
#R CMD BATCH [options] [infile] [outfile]       #在Linux上运行R脚本的命令
#--在Windonws上运行R脚本的命令
...
##################################%%%%%%%%%%%%%%%%%%%%%Advanced R(Hadley)%%%%%%%%%%%%%%%%%%%%%########################################################
##################################1. Names and values##################################
library(lobstr)       #used to dig into the detail of an object.
...
`111` <- y
z <- c(1,2,3)
# check the memery address of objects
# x, y,`111` are binded to the same memory address where c(1,2,3) is stored
obj_addr(x)    
obj_addr(y)
obj_addr(`111`)
obj_addr(z)        #note: address of z is defferent from other objects
...
# R objects(only some of them,e.g. vector) are unchangeable
x <- c(1,2,3)
y <- x                       
obj_addr(x) == obj_addr(y)   #shared binding
cat(tracemem(y),"\n")
y[[3]] <- 4                  #copy-on-modify
y[[3]] <- 7                  #copy-on-modify again
untracemem(y)


# SHALLOW COPY (list, data frame(list of vectors))
#list sample
l1 <- list(1,2,3)
l2 <- l1
obj_addr(l1) == obj_addr(l2)     #shared binding
cat(tracemem(l2),"\n")
l2[[2]] <- 7
obj_addr(l1) == obj_addr(l2)
obj_addr(l1[[1]]) == obj_addr(l2[[1]])   #TRUE, only the second element of l1 is copy-on-modify
ref(l1,l2)                               #To see values that are shared binding across lists
untracemem(l2)

# data frame sample
d1 <- data.frame(x = c(1, 5, 6), y = c(2, 4, 3))
d2 <- d1
d2[, 2] <- d2[, 2] * 2                  #modify on column(shallow copy)
obj_addr(d1[,1]) == obj_addr(d2[,1])    #TRUE, shallow copy only the second column
d3 <- d1                  
d3[1,] <- c(7,7)                        #modify on row(deep copy)
obj_addr(d1[,1]) == obj_addr(d3[,1])    #FALSE, deep copy entire object

# GLOBAL STRING POOL - character vector
x <- c("a", "a", "abc", "d")
y <- c("a","abc","abc","e")
ref(x,y,character = T)
...
obj_size(x)
y <- list(x,x,x)
obj_size(y)                      #becuase the elements of lists are references to values, so y is only 80 bytes larger than x
obj_size(list(NULL,NULL,NULL))   #this object is 80 bytes - the size of an empty list with three elements
...
tracemem(v1)
v1[[2]] <- 7
obj_addr(v1)       #the address changed? why? there are two complications make it hard to predict whether or not a copy will occur
...
e2$c <- 4
obj_addr(e2)                       #object address didn't change

# GC(garbage collector)
gcinfo(TRUE)              #find out when GC will run
gc()                      #force GC
mem_used() / 1024 / 1024
...
#Vectors has two most important attributes: dimension & class

#There are four common types of atomic vectors: logical, integer, double, and character. 
v1 <- c(0.1234,1e10,Inf,-Inf,NaN,NA,NULL)         #c() stands for combine
...
# ATTRIBUTES
#Important Note: data structures like matrix, array, factors and date/times are built on top of atomic vectors by adding attributes.
...
# DIMENSIONS
a <- array(c(1:12), dim = c(2,3,4))
b <- matrix(c(1:12), nrow = 3, ncol = 4)
class(a); class(b)
c <- c(1:12)
dim(c) <- c(3,4)
c; class(c); str(c); 

##################################2.1. S3 ATOMIC VECTOR ##################################
...
# note the deferent between character vector and factor
sex_char <- c("m", "m", "m")
...
# DATE - are built on top of double vectors.
...
# DATETIME - Base R provides two ways of storing date-time information, POSIXct, and POSIXlt. 
# ct: calendar time; lt: local time
now_ct <- as.POSIXct(Sys.time(), tz = "CST")
now_ct
...
# DURATION - are built on top of doubles, and have a units attribute that determines how the integer should be interpreted
...
##################################2.2. LIST ##################################
#an element of a list can be any type, and the elements of a list are references
l1 <- list(
  1:3, 
  "a", 
  c(TRUE, FALSE, TRUE), 
...
l5 <- c(list(1,"a", 2), c(3, 4)); str(l5)

##################################2.3. DATA FRAME & TIBBLES ##################################
...
sum; typeof(sum)        #type: builtin
`[`; typeof(`[`)        #type: special
body(sum); formals(sum); environment(sum)
...
identical(current_env(), global_env())         #to compare environments, use identical(), not == (== is a vectorised operator, environment is not a vector)
...
e2a <- env(d = 4, e = 5)
e2b <- env(e2a, a = 1, b = 2, c = 3)       #第一个参数是'parent env', if 没有设置，默认为current_env
...
#Learn from following code:
#1. ancestors of the global environment include every attached package
#2. The ancestors of every environment eventually terminate with the empty environment. 
...
#SEARCH PATH
search_envs()
#the last two environments:
#1. Autoloads: uses delayed bindings to save memory by only loading package objects (like big datasets) when needed
#2. package:base:
...
g(10)
#note: 
#1. The above function returns the same value every time because of the fresh start principle. Each time a function is called, a new environment is created to host execution. 
#2. This is called the execution environment, and its parent is the function environment. 

##################################5.3. The caller environment##################################
# CALLER Environment
...
fw <- function() {
  cat("1\n")
  warning("W1", call. = FALSE)
  cat("2\n")
  warning("W2")
  cat("3\n")
  warning("W3")
}
fw() #By default, warnings are cached and printed only when control returns to the top level, change the options("warn") can change this behavious
...
#Note: the defferent between 'message()' and 'cat()'
cat("Hi\n")
message("Hi")
...
m_cnd <- catch_cnd(warning("This is a warning"))
str(m_cnd)
...
f3 <- function(x){
  tryCatch(
    ...
    error = function(cnd) NA,          #exiting handlers, and note this is a function with single argument (the condition object)
    finally = {
      print("Code run to here")
    }
  )
}
f3(1)
...
tryCatch(
  { 
    ...
    print(2 + 2)                    #this won't be printed out
  },
...
log("c")
log(10, base = "a")
#the error message are the same, not specific.

m_log <- function(x, base = exp(1)){
  if(!is.numeric(x)){
    abort(paste0("'x' must be a numeric vector; not ", type_of(x), "."))
  }
  if(!is.numeric(base)){
...
# Three functional programming tools are:
#1. functionals
...
square <- power1(2)     #manufactured functions
cube <- power1(3)
fn_env(square); env_parents(fn_env(square)); fn_env(square)$exp
square(3); cube(3)
...
# SLOOP PACKAGE
sloop::otype(mtcars)      # return the OOP system used
sloop::otype(c(1:10))
...
f <- factor(c("a","b","c"))
typeof(f)                     #return the base type of object
attr(f,"class")               #class attribute
attr(f, "levels")             #levels attribute
attributes(f)
unclass(f)                    #get the “underlying” base type, strip the class attribute
typeof(unclass(f))
...
#generic function, e.g. print, str...
ftype(print)
print(f)
print(unclass(f))

times <- strptime(c("2017-01-01", "2020-05-04 03:21"), format = "%Y-%m-%d")
str(times)
str(unclass(times))

#The generic is a middleman: its job is to define the interface (i.e. the arguments) then find the right implementation for the job. 
...
s3_dispatch(print(f))         #note: S3 methods are functions with a special naming scheme, generic.class()
s3_dispatch(str(times))
s3_dispatch(print(unclass(f)))
s3_dispatch(str(unclass(times)))

print.factor(f)     #never do this, generic will find corresponding s3 method
print(f)

##################################10.2 Classes##################################
#S3 has no formal definition of a class: to make an object an instance of a class, you simply set the class attribute.
x <- structure(list(a = c(1:3), b = c("a","b","c")),
               class = "my_class")
str(x)
class(x) <- "my_class"

class(x)    #return the class of S3 object
inherits(x, "my_class")     #check if the object 'x' is an instance of a class
inherits(x, "factor")

#How about change the class of an existing object?
mod <- lm(log(mpg)~log(disp), data = mtcars)
class(mod)    #the class of 'mod' is 'lm'
print(mod)
class(mod) <- "Date"         #change the class of 'mod' from 'lm' to 'Date'
print(mod)


#Three functions required when creating new class:
#1. constructor
#2. validator
#3. helper

# CONSTRUCTOR
#sample of making a constructor for the simplest S3 class:Date
#A 'Date' is just a double with a 'Date' class attribute
new_Date <- function(x = double()){
  stopifnot(is.double(x))
  structure(x, class = "Date")
}
o_date <- new_Date(c(-1,0,1,100))
typeof(o_date)
class(o_date)
attributes(o_date)

#sample: 'difftime' constructor
new_timediff <- function(x = double(), units = "secs"){
  stopifnot(is.double(x))
...
o_timediff <- new_timediff(10, units = "hours")
typeof(o_timediff); class(o_timediff); attributes(o_timediff)
...
#Two most important arguments of R6Class():
#1. The first argument is the 'classname'
#2. The second argument - public - supplies a list of methods and fields. (note: fields and methods as opposed to variables and functions)
...
# METHOD CHAINING
y <- Accumulator$new()
y$add(10)$add(10)$add(10)$sum        #method 'add' always return self invisiblely (method chaining)
...
#2. $print() allows you to override the default printing behaviour.
...
andy <- Person$new("Andy", 30)
andy
andy$age
andy$print()

# ADD METHOD AFTER CREATION
Person$set("public", "birth", NULL)
Person$set("public", "getBirth", function(NOW = Sys.Date()){
  self$birth <- lubridate::year(Sys.Date()) - self$age
  invisible(self)
})
andrew <- Person$new("Andrew", 31)
andrew$getBirth()$birth

Person_China <- R6Class("Person_China",
                        inherit = Person,
                        public = list(
                          getBirth = function(NOW = Sys.Date()){
                            self$birth <-  max(lubridate::year(Sys.Date()) - self$age - 10, 1) 
                            invisible(self)
                          }
                        ))
tim <- Person_China$new("tim", 100)
tim$getBirth()$birth
class(tim)
names(tim)
...
i <- function(d){
  if(!is.numeric(d)){
    stop("'d' must be numberic", call. = FALSE)
  }
  d + 10
}
f("a")
...
f(j())      #parameter of f is j(), will ba lazy evaluation
traceback()
rlang::with_abort(f(j()))          #rlang::last_trace() is ordered in the opposite way to traceback()
summary(rlang::last_error())


# RERUN WITH DEBUG
#Rerun with debug - returns the command that created the error, pausing execution where the error occurred. 
g <- function(b){     #add a call to browser()(where you want to pause) in function g(), 
  browser()
  h(b)
}
h <- function(c){
...
# Operations in browser tool bar:
#1. Next(n): executes the next step in the function
#2. Step into(s): works like next, but if the next step is a function, it will step into that function so you can explore it interactively
#3. Finish(f): finishes execution of the current loop or function
#4. Continue(c): leaves interactive debugging and continues regular execution of the function.
#5. Stop(Q): stops debugging, terminates the function, and returns to the global workspace.


#Alternatives of 'browser'
#1. Breakpoint
#2. recover() - options(error = recover)
#3. debug()
...
# PROFILING
#R uses a fairly simple type called a sampling or statistical profiler to understand code performance.
