...
############################################################Model Infors############################################################
#append(mR, list(list(...)))
mR <- list(list(md_name = "template", 
                md_type = "template",
                md_store = "template",
                md_paras = list(),
                md_otherInfo = list(),
                md_metrics = data.frame()))

############################################################Original data load############################################################
org_train <- read_csv("./data/train.csv")
org_test <- read_csv("./data/test.csv")
nrow(org_train);  #903653
nrow(org_test);   #804684
#org_all <- read_csv("./data/all.csv")
#org_all <- rbind(org_train %>% mutate(flag = 1),org_test %>% mutate(flag = 0))
#nrow(org_all)
...
org_train <- parse(org_train)
org_test <- parse(org_test)

setdiff(names(org_train), names(org_test))

org_train <- org_train %>% select(-tfcS_campaignCode)

#--remove features which only have 1 value
ft_uniqValue <- sapply(org_train, n_distinct)
(ft_rm <- names(ft_uniqValue[ft_uniqValue==1]))
org_train <- org_train %>% select(-ft_rm)
org_test <- org_test %>% select(-ft_rm)

#--check missing values
is_na_val <- function(x) x %in% c("not available in demo dataset", "(not provided)",
                                  "(not set)", "<NA>", "unknown.unknown",  "(none)")

#org_train %>% mutate_all(funs(ifelse(is_na_val(.),NA,.)))
#org_test %>% mutate_all(funs(ifelse(is_na_val(.),NA,.)))

#combine test and train dataset into all
org_train <- org_train %>% mutate(flag = 1); names(org_train); names(org_test)
org_test <- org_test %>% 
  mutate(tt_transactionRevenue = NA, flag = 0) %>% 
  select(1:32,tt_bounces,tt_newVisits,tt_transactionRevenue,flag)

if(all(names(org_train) == names(org_test))) all <- rbind(org_train,org_test)
