...
p_home <- v_51job_jobSearch_home
p_jobArea <- attr(gv_cities,"51job_code")[which(gv_cities == "成都")]
p_issuedate <- v_51job_issuedateCode[3]                   #Delta loading - issue date: last week
p_providesalary <- attr(gv_salaryRange,"51job_code")[which(gv_salaryRange == "所有")]
p_cotype <- attr(gv_companyType,"51job_code")[which(gv_companyType == "所有")]
p_companysize <- attr(gv_companySize,"51job_code")[which(gv_companySize == "所有")]
count_industries <- nrow(gv_industries)
week_end <- gsub("-","",as.character(floor_date(Sys.Date(), "week") + 7))
not_51job_industry_code <- c("00")

for(i in 1:count_industries){
  p_industry <- gv_industries$sub_industry_code_51job[i]
  if(any(p_industry == not_51job_industry_code)){
    print("No this industry")
  }else{
    p_sub_industry_en <- gv_industries$sub_industry_en[which(gv_industries$sub_industry_code_51job == p_industry)[1]]
    ...
    if(!is.na(p_industry)){
      ...
      if(count_df_51urls > 0){
        uf_xlsxDataExport(df_companies_51urls,paste0("./Job51_Weekly_Delta_Urls/",
                                                     "Companies_51urls_",
                                                     attr(gv_cities,"city_en")[which(attr(gv_cities,"51job_code") == p_jobArea)[1]],
                                                     "_Week_",week_end,
                                                     ".xlsx")
                          ,gv_industries$sub_industry_en[which(gv_industries$sub_industry_code_51job == p_industry)[1]]
                          ,TRUE)
      }
...
for(j in 1:count_industries){
  p_industry <- gv_industries$sub_industry_code_51job[j]
  if(p_industry != "00"){
    p_sheet_import <- gv_industries$sub_industry_en[which(gv_industries$sub_industry_code_51job == p_industry)[1]]
